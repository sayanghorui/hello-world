...
def trainBayes(sentencesTrain, pWordPos, pWordNeg, pWord):
    ...
    for sentence, sentiment in sentencesTrain.items():
        ...
        trigramList=bigramList.copy()  # initialise trigramList
        ...
        for word in trigramList: #calculate over trigrams
...
def testDictionary(sentencesTest, dataName, sentimentDictionary, threshold):
    ...
    recall_neg=correctneg/float(totalneg)
    f_pos=2*precision_pos*recall_pos/(precision_pos+recall_pos);
    f_neg=2*precision_neg*recall_neg/(precision_neg+recall_neg+1);
...
def mostUseful(pWordPos, pWordNeg, pWord, n):
    ...
    #counting number of n-gram words in most useful list, which are present in sentiment dictionary
    top = head + tail
    count = 0
    for items in top:
        if items in sentimentDictionary:
            count+= 1
    print('\ntotal most useful n-grams in dictionary: %s'  %count)
...
#testDictionaryModified(sentencesTrain,  "Films (Train Data, Rule-Based)\t", sentimentDictionary, -4)
#testDictionaryModified(sentencesTest,  "Films (Train Data, Rule-Based)\t", sentimentDictionary, -4)
#testDictionaryModified(sentencesNokia,  "Films (Train Data, Rule-Based)\t", sentimentDictionary, -3)
